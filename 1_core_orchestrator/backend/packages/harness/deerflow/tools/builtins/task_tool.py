...
def task_tool(
    ...
    description: str,
    prompt: str,
    subagent_type: Literal["imaging-agent", "medical-knowledge-agent"],
    ...
    - **imaging-agent**: 影像科Agent，负责调用MCP服务识别和分析医疗影像图片（X光、CT、MRI等）。
      当用户上传医疗影像需要专业分析时使用。
    - **medical-knowledge-agent**: 医疗知识检索Agent，在主Agent的网络搜索无法找到足够医疗知识时调用，
      进行更深入的专业医疗知识检索。
    ...
    if config is None:
        return f"Error: Unknown subagent type '{subagent_type}'. Available: imaging-agent, medical-knowledge-agent"
